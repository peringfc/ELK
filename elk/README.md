...
[![Versão do Elastic Stack](https://img.shields.io/badge/Elastic%20Stack-8.6.1-00bfb3?style=flat&logo=elastic-stack)](https://www.elastic.co/ blog/categoria/lançamentos)
...
[![Entre no chat](https://badges.gitter.im/Join%20Chat.svg)](https://app.gitter.im/#/room/#deviantony_docker-elk:gitter.im)
...
![Demonstração animada](https://user-images.githubusercontent.com/3299086/155972072-0c89d6db-707a-47a1-818b-5f976565f95a.gif)

> **Nota**
> Os recursos [Platinum][assinaturas] são ativados por padrão por um período de [avaliação][license-mngmt] de **30 dias**. Depois
> neste período de avaliação, você manterá o acesso a todos os recursos gratuitos incluídos na licença Open Basic sem problemas,
> sem necessidade de intervenção manual e sem perda de dados. Consulte a seção [Como desabilitar serviços pagos
> features](#how-to-disable-paid-features) para desativar esse comportamento.
...
* [`tls`](https://github.com/deviantony/docker-elk/tree/tls): criptografia TLS habilitada no Elasticsearch, Kibana (opt in),
   e Frota
* [`searchguard`](https://github.com/deviantony/docker-elk/tree/searchguard): suporte do Search Guard

---

## Filosofia

Nosso objetivo é fornecer a entrada mais simples possível na pilha Elastic para qualquer pessoa que queira experimentar
esta poderosa combinação de tecnologias. A configuração padrão deste projeto é propositalmente mínima e sem opinião. Isto
não depende de nenhuma dependência externa e usa o mínimo de automação personalizada necessário para colocar as coisas em funcionamento e
correndo.

Em vez disso, acreditamos em uma boa documentação para que você possa usar este repositório como modelo, ajustá-lo e torná-lo _seu
ter_. [sherifabdlnaby/elastdocker][elastdocker] é um exemplo entre outros de projeto que se baseia nessa ideia.
...
1. [Requisitos](#requirements)
    * [Configuração do host](#configuração do host)
    * [Docker Desktop](#docker-desktop)
      * [Janelas](#janelas)
      * [macOS](#macos)
1. [Uso](#uso)
    * [Bringing-up-the-stack](#bring-up-the-stack)
    * [Configuração inicial](#configuração inicial)
      * [Configurando a autenticação do usuário](#setting-up-user-authentication)
      * [Injetando dados](#injecting-data)
    * [Limpeza](#limpeza)
    * [Seleção de versão](#version-selection)
1. [Configuração](#configuração)
    * [Como configurar o Elasticsearch](#como-configurar-o-elasticsearch)
    * [Como configurar o Kibana](#como-configurar-kibana)
    * [Como configurar o Logstash](#como-configurar-logstash)
    * [Como desativar recursos pagos](#how-to-disable-paid-features)
    * [Como expandir o cluster do Elasticsearch](#how-to-scale-out-the-elasticsearch-cluster)
    * [Como reexecutar o setup](#how-to-re-execute-the-setup)
...
1. [ajuste JVM](#jvm-tuning)
    * [Como especificar a quantidade de memória usada por um serviço](#how-to-specific-the-amount-of-memory-used-by-a-service)
    * [Como habilitar uma conexão JMX remota para um serviço](#how-to-enable-a-remote-jmx-connection-to-a-service)
1. [Indo além](#indo-mais adiante)
    * [Plugins e integrações](#plugins-and-integrations)
...
* [Docker Compose][compose-install] versão **1.26.0** ou mais recente (incluindo [Compose V2][compose-v2])
* 1,5GB de RAM

> **Atenção**
> Embora as versões do Compose entre **1.22.0** e **1.25.5** também possam executar tecnicamente esta pilha, essas versões têm um
> [problema conhecido](https://github.com/deviantony/docker-elk/pull/678#issuecomment-1055555368) que os impede de
> analisando valores entre aspas corretamente dentro de arquivos `.env`.

> **Nota**
> Especialmente no Linux, certifique-se de que seu usuário tenha as [permissões necessárias] [linux-postinstall] para interagir com o Docker
> demônio.

Por padrão, a pilha expõe as seguintes portas:

* 5044: Entrada Logstash Beats
* 50000: Logstash entrada TCP
...
> **Atenção**
> [bootstrap-checks][bootstrap-checks] do Elasticsearch foram propositadamente desativados
